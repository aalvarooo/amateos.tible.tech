...
<body>
    ...
    var_dump($_FILES);
    $imagen_subida = null;
    if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST["nombre"]) && !empty($_POST["apellidos"]) && !empty($_POST["email"]) && !empty($_POST["telefono"]) && !empty($_FILES["imagen"])) {
        ...
        <div class="container-md mt-5">
            <ul class="list-group">
                <li class="list-group-item">Gracias <?=($_POST["nombre"]&& $_POST["apellidos"])?></p>
                ...
                <?php if ($imagen_subida): ?> 
                    <li class="list-group-item">has subido la siguiente imagen <?=($_FILES["imagen"]["type"])?></p>
                ...
                <div class="col-12 col-md-7 ">
                    <form action="" method="POST" class="me-5" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control fs-5 <?= isset($_POST["nombre"]) && empty($_POST["nombre"]) ? 'is-invalid' : '' ?>" name="nombre" value="<?php echo isset($_POST["nombre"]) ? $_POST["nombre"] : '' ?>">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Apellidos</label>
                            <input type="text" class="form-control <?=isset($_POST["apellidos"]) && empty($_POST["apellidos"]) ? 'is-invalid' : '' ?>" name="apellidos" value="<?php echo isset($_POST["apellidos"]) ? $_POST["apellidos"]:''?>">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Email</label>
                            <input type="email" class="form-control <?=isset($_POST["email"]) && empty($_POST["email"]) ? 'is-invalid' : '' ?>" name="email" value="<?php echo isset($_POST["email"])? $_POST["email"]:''?>">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Teléfono</label>
                            <input type="text" class="form-control <?=isset($_POST["telefono"]) && empty($_POST["telefono"]) ? 'is-invalid' : '' ?>" name="telefono" value="<?php echo isset($_POST["telefono"])? $_POST["telefono"]:''?>">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Imagen</label>
                            <input type="file" class="form-control <?=isset($_POST["imagen"]) && empty($_POST["imagen"]) ? 'is-invalid' : '' ?>" name="imagen" value="<?php echo isset($_POST["imagen"])? $_POST["imagen"]:''?>">
                        </div>
                        <div>
